...
static __u64 load_image_to_memory(const char *path, __u64 load_paddr) {
    ...
    log_info("[MULTIBOOT] Loading image from: %s to paddr: 0x%llx", path, load_paddr);

    image_content = read_file(path, (uint64_t *)&size);
    map_size = load_buffer_to_memory(image_content, size, load_paddr);

    log_info("[MULTIBOOT] Image loaded, size: %llu, mapped: %llu", size, map_size);
...
static void *build_multiboot2_info(uint64_t total_mem_size, const char *cmdline,
                                   memory_region_t *mem_regions, int num_regions,
                                   uint64_t initramfs_gpa, uint64_t initramfs_size,
    ...
    size_t mmap_tag_size = sizeof(struct multiboot_tag_mmap) +
    ...
    if (has_initramfs) {
        ...
        module_tag_size = 8 + 8 + ((cmdline_len + 7) & ~7);  // header(8) + mod_start/mod_end(8) + padded cmdline
    }
    ...
    // Memory info tag (required by Asterinas)
    {
        ...
        *mem_upper = (total_mem_size > 0x100000) ? (total_mem_size / 1024 - 1024) : 0;  // KB above 1MB

        log_info("[MULTIBOOT] Added basic meminfo tag: lower=%u KB, upper=%u KB", *mem_lower, *mem_upper);
        tag_ptr += 16;
    }
    ...
    {
        struct multiboot_tag_mmap *mmap_tag = (struct multiboot_tag_mmap *)tag_ptr;
        ...
        for (int i = 0; i < num_regions; i++) {
            if (mem_regions[i].type == MEM_TYPE_RAM) {
                ...
                if (region_start < LOW_MEM_TOP) {
                    ...
                    uint64_t low_end = (region_end < LOW_MEM_TOP) ? region_end : LOW_MEM_TOP;
                    uint64_t low_size = low_end - low_start;

                    if (low_size > 0) {
                        mmap_tag->entries[entry_idx].addr = low_start;
                        mmap_tag->entries[entry_idx].len = low_size;
                        mmap_tag->entries[entry_idx].type = MULTIBOOT_MEMORY_AVAILABLE;
                        mmap_tag->entries[entry_idx].zero = 0;
                        log_info("[MULTIBOOT] Memory map entry %d: addr=0x%llx (GPA), len=0x%llx, type=AVAILABLE (low)",
                ...
                if (region_end > LOW_MMIO_TOP) {
                    uint64_t high_start = (region_start > LOW_MMIO_TOP) ? region_start : LOW_MMIO_TOP;
                    ...
                    if (high_size > 0) {
                        ...
                        mmap_tag->entries[entry_idx].type = MULTIBOOT_MEMORY_AVAILABLE;
                        mmap_tag->entries[entry_idx].zero = 0;
                        log_info("[MULTIBOOT] Memory map entry %d: addr=0x%llx (GPA), len=0x%llx, type=AVAILABLE (high)",
    ...
    if (cmdline && strlen(cmdline) > 0) {
        ...
        *tag_size_field = tag_size;
        ...
        log_info("[MULTIBOOT] Added cmdline tag: '%s' (size: %zu)", cmdline, tag_size);
    ...
    if (has_initramfs) {
        ...
        *mod_end = (uint32_t)((initramfs_gpa + initramfs_size) & 0xFFFFFFFF);
        ...
        log_info("[MULTIBOOT] Added initramfs module tag: start=0x%llx, end=0x%llx, cmd='%s'",
                 (unsigned long long)initramfs_gpa,
                 (unsigned long long)(initramfs_gpa + initramfs_size),
                 mod_cmd);
    ...
    *total_size_ptr = total_size;
    ...
    log_info("[MULTIBOOT] Multiboot2 info built: total_size=%u bytes", total_size);
...
/**
 * @brief Load ELF segments to correct physical addresses with GPA-to-HPA translation
 *
 * This function parses the ELF file and loads each PT_LOAD segment
 * to its specified physical address (p_paddr) plus an offset.
 *
 * @param elf_path Path to ELF file
 * @param total_size Output: total size of loaded segments (max p_paddr + p_memsz)
 * @param gpa_to_hpa_offset Offset to add to p_paddr for GPA-to-HPA translation
 * @return 0 on success, -1 on error
 */
static int load_elf_segments(const char *elf_path, uint64_t *total_size, int64_t gpa_to_hpa_offset) {
    fprintf(stderr, "[ELF] load_elf_segments called with path: %s, gpa_to_hpa_offset=0x%llx\n",
    ...
    fprintf(stderr, "[ELF] Program headers: offset=0x%llx, entry_size=%u, count=%u\n",
    ...
    *total_size = 0;
    ...
    for (int i = 0; i < e_phnum; i++) {
        ...
        uint64_t p_offset = *(uint64_t *)((char *)elf + e_phoff + i * e_phentsize + 8);
        uint64_t p_paddr = *(uint64_t *)((char *)elf + e_phoff + i * e_phentsize + 24);
        uint64_t p_filesz = *(uint64_t *)((char *)elf + e_phoff + i * e_phentsize + 32);
        uint64_t p_memsz = *(uint64_t *)((char *)elf + e_phoff + i * e_phentsize + 40);

        fprintf(stderr, "[ELF] Segment %d: type=%u, p_offset=0x%llx, p_paddr(GPA)=0x%llx, p_filesz=0x%llx\n",
        ...
        if (p_offset + p_filesz > file_size) {
            fprintf(stderr, "[ELF] Segment %d file offset+size exceeds file size\n", i);
            free(elf);
            return -1;
        }
        ...
        if (p_filesz > 0) {
            fprintf(stderr, "[ELF] Loading segment %d to HPA 0x%llx (GPA 0x%llx), size 0x%llx\n",
                     i, load_paddr, p_paddr, p_filesz);
            // load_buffer_to_memory exits on failure, so we don't need to check return value
            load_buffer_to_memory(
                (char *)elf + p_offset,
                p_filesz,
                load_paddr
            );
        ...
        if (p_memsz > p_filesz) {
            ...
            fprintf(stderr, "[ELF] Zero-filling segment %d: HPA 0x%llx, size 0x%llx (BSS/stack)\n",
                     i, zero_fill_start, zero_fill_size);
            // Allocate a zero buffer and load it
            void *zero_buf = calloc(1, zero_fill_size);
            if (!zero_buf) {
                fprintf(stderr, "[ELF] Failed to allocate zero buffer for segment %d\n", i);
                free(elf);
                return -1;
            }
...
static int zone_start_from_json(const char *json_config_path,
    ...
    fprintf(stderr, "[DEBUG] zone_start_from_json: opening file: %s\n", json_config_path);
    ...
    fprintf(stderr, "[DEBUG] zone_start_from_json: file_size=%ld\n", file_size);
    ...
    fprintf(stderr, "[DEBUG] zone_start_from_json: parsing JSON\n");
    ...
    if (config->multiboot_enabled && num_memory_regions > 0) {
        ...
        for (int i = 0; i < num_memory_regions; i++) {
            ...
            if (mem_region->type == MEM_TYPE_RAM) {
                gpa_to_hpa_offset = (int64_t)mem_region->physical_start - (int64_t)mem_region->virtual_start;
                fprintf(stderr, "[MULTIBOOT] GPA-to-HPA offset: 0x%llx (HPA=0x%llx, GPA=0x%llx)\n",
    ...
    if (config->multiboot_enabled) {
        ...
        int ret = load_elf_segments(kernel_filepath_json->valuestring, &total_size, gpa_to_hpa_offset);
        fprintf(stderr, "[MULTIBOOT] load_elf_segments returned: %d, total_size: 0x%llx\n", ret, total_size);
        fflush(stderr);
        if (ret != 0) {
...
// x86_64 uses ACPI
    ...
    if (config->multiboot_enabled) {
        ...
        cJSON *mb_info_paddr_json = cJSON_GetObjectItem(root, "multiboot_info_paddr");
        ...
        if (find_elf_entry_point(kernel_filepath_json->valuestring, &elf_entry) == 0) {
            fprintf(stderr, "[MULTIBOOT] Found ELF entry point (GPA): 0x%llx\n", elf_entry);
            // Entry point is GPA, keep as-is for hvisor
            config->entry_point = elf_entry;
            fprintf(stderr, "[MULTIBOOT] Entry point set to: 0x%llx\n", config->entry_point);
        } else {
        ...
        cJSON *initramfs_filepath_json = cJSON_GetObjectItem(root, "initramfs_filepath");
        cJSON *initramfs_load_gpa_json = cJSON_GetObjectItem(root, "initramfs_load_gpa");
        if (initramfs_filepath_json != NULL && initramfs_load_gpa_json != NULL
            ...
            uint64_t initramfs_hpa = (uint64_t)((int64_t)initramfs_gpa + gpa_to_hpa_offset);
            fprintf(stderr, "[MULTIBOOT] Loading initramfs: %s to GPA=0x%llx (HPA=0x%llx)\n",
                    initramfs_filepath_json->valuestring, initramfs_gpa, initramfs_hpa);
            initramfs_size = load_image_to_memory(initramfs_filepath_json->valuestring, initramfs_hpa);
        ...
        void *mb_info = build_multiboot2_info(total_mem, cmdline, config->memory_regions, num_memory_regions,
        ...
        fprintf(stderr, "[MULTIBOOT] Loading Multiboot2 info to HPA: 0x%llx (GPA: 0x%llx)\n",
                  mb_info_hpa, mb_info_gpa);

        // Load Multiboot info to memory at HPA (use larger size to accommodate tags)
        ...
        fprintf(stderr, "[MULTIBOOT] Multiboot disabled, using default boot mode\n");
    }
    // ============================================================
...
static int zone_start(int argc, char *argv[]) {
    ...
    fprintf(stderr, "[DEBUG] zone_start: argc=%d\n", argc);
    if (argc != 4) {
        help(1);
    }
    json_config_path = argv[3];
    fprintf(stderr, "[DEBUG] zone_start: json_config_path=%s\n", json_config_path);

    memset(&config, 0, sizeof(zone_config_t));

    fprintf(stderr, "[DEBUG] zone_start: opening device\n");
    fd = open_dev();
    fprintf(stderr, "[DEBUG] zone_start: fd=%d\n", fd);
    ret = ioctl(fd, HVISOR_CONFIG_CHECK, &hvisor_config_version);
    fprintf(stderr, "[DEBUG] zone_start: ioctl returned ret=%d, version=0x%x\n", ret, (unsigned int)hvisor_config_version);
    close(fd);
    ...
    if (hvisor_config_version != CONFIG_MAGIC_VERSION) {
        fprintf(stderr, "[DEBUG] version mismatch! tool=0x%x, hvisor=0x%x\n", 
                CONFIG_MAGIC_VERSION, (unsigned int)hvisor_config_version);
    ...
    fprintf(stderr, "[DEBUG] calling zone_start_from_json\n");
...
int main(int argc, char *argv[]) {
    ...
    fprintf(stderr, "[DEBUG] hvisor-tool starting, argc=%d\n", argc);
    ...
    log_info("[DEBUG] Log system initialized");
